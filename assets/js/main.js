...
var cvform='<div id="forms_login" class="forms_cv"><a class="close" style="cursor:pointer;" onclick="call();">Close</a>';
    ...
    $(document).ready(function(){
		   ...
		   $('input[name=submission_num]').blur(function(){
					...
					$('#table1').append('<input type="hidden" name="count_r" value="1">');
					$('#table2').append('<input type="hidden" name="count_rr" value="1">');
					$('#table3').append('<input type="hidden" name="count_rrr" value="1">');
					
					sliders(1,0,$(this).val(),'qty1',0,"r");					
		   ...
		   $('#add_anchor2').click(function() {
					...
					$('input[name=count_rr]').val(nums);
					$('#rr').append('<input type="hidden" name="min_qty_rr'+nums+'" value="'+old_val+'">');
		  ...
		  $('#third_anchor').click(function(){
		   ...
		   $('#add_anchor3').click(function() {
					...
					//sliders(nums,$('input[name=submission_num]').val(),'qlty'+nums); //old logic for all slider is same
					//$('#min_qty'+nums).val(old_val);
					$('input[name=count_rrr]').val(nums);
					$('#rrr').append('<input type="hidden" name="min_qty_rrr'+nums+'" value="'+old_val+'">');
	...
	//$('#spin_area').html(spiner);
	       $('#chk_box1 input:checkbox').click(function(){
					...
					/*function progid(value)
       {        
               site_url = "<?=$site_url?>";
               $("#sel_pkg > option").remove();
               $.ajax({
                       url: site_url+'package/getProgData/'+value,
                       success: function(task_data)
                       {
                               arr_task_data = task_data.split('@^^@');                        
                               for (i=0;i<arr_task_data.length;i++)
                               {
                                       var optn = document.createElement("OPTION");
                                       optn.text = arr_task_data[i];
                                       optn.value = arr_task_data[i];
                                       document.pkgFrm.sel_pkg.options.add(optn);
                               }
                       }
               });
       }*/
					...
					$("#dropdown").find('option').remove();
                     for (i=0;i<result.length;i++)
						...
						$("#dropdown").append('<option value="'+result [i]+'">'+result [i]+'</option>');
	          ...
	          /*$('#select_all').click(function () {
				...
				$('.col-md-6 #right input:checkbox').click(function(){
					$('#spin_area_smart').css('display','none');
					$('#spin_area_manually').removeAttr('style');
					$('.col-md-6 #left input:checkbox').prop('checked', false);
					$('.part5_3').find('h3').text('How many unique articles should we create/ post?');
				});
				
				$('.col-md-6 #left input:checkbox').click(function(){
					$('#spin_area_manually').css('display','none');
					$('#spin_area_smart').removeAttr('style');
					$('.col-md-6 #right input:checkbox').prop('checked', false);
					$('.part5_3').find('h3').text('# ofSubmissions:');
				});
				...
				$( "#datepicker" ).datepicker({				
					 showOn: 'button',      
					  buttonImage: path_url+'http://axattechnologies.in/demo/serp_avenger/assets/images/pic22.png',
					  buttonImageOnly: true,					  
					  dateFormat: 'mm/dd/yy',
					});
				
				$('#start_timing input:radio:first').click(function(){					
					if($(this).is(":checked")){						
					$('#start_timing input:radio:last').removeAttr('checked');
					$('.cal').css('display','none');
					$('#datepicker').val('');
					}
				});
				$('#start_timing input:radio:last').click(function(){					
					if($(this).is(":checked")){						
					$('#start_timing input:radio:first').removeAttr('checked');
			  ...
			  $('input:checkbox[name=unique_comments]').click(function(){
				 ...
				 if($(this).is(":checked")){
				     $('#unique_key').html('<input type="text" name="keywords" value="" placeholder="Keyword"/>');	 
				  // $('body').append(cvform);				  
			 ...
			 $('input[name=post_title]').blur(function(){				 
					 $.post( path_url+"scrapper/valid_spintax", { chk_value:$(this).val(), action:'vti'}, function(data){						//alert('fsafsf');
						if(data=='yes'){
						    $('#valid-title').html('<img src="http://axattechnologies.in/demo/serp_avenger/assets/images/pic11.png">&nbsp;&nbsp;Correct Spintax Detected');
						}else if(data=='no'){
							$('#valid-title').html('<img src="http://axattechnologies.in/demo/serp_avenger/assets/images/img18.png">&nbsp;&nbsp;Correct Spintax Detected');
						}
					});					
				});
			 $('textarea[name=post_content]').blur(function(){				 
					 $.post( path_url+"scrapper/valid_spintax", { chk_value:$(this).val(), action:'vti'}, function(data){						//alert('ghjkhkghk'); 					
						if(data=='yes'){
						    $('#valid-post').html('<img src="http://axattechnologies.in/demo/serp_avenger/assets/images/pic11.png">&nbsp;&nbsp;Correct Spintax Detected');
						}else if(data=='no'){
							$('#valid-post').html('<img src="http://axattechnologies.in/demo/serp_avenger/assets/images/img18.png">&nbsp;&nbsp;Correct Spintax Detected');
						}
					});					
				});
