...
class Am_Storage_Selectel extends Am_Storage
{
        ...
        function onSetupForms(Am_Event_SetupForms $event)
    {
        ...
        $form->addText('access_key', array('size' => 40))->setLabel('Your account login')
            ->addRule('required');
        $form->addPassword('secret_key', array('size' => 40))->setLabel(array('Password for Cloud Storage', '(separate password then for Control Panel)'))
            ->addRule('required');
        ...
        if ($this->isConfigured())
        {
            ...
            $form->addSelect('links_container', '', array('options' => array_combine($containers, $containers)))
                ->setLabel(array('Container for links', 'aMember will create links in the following format: http://yourcloudstorageurl.com/CONTAINERNAME/uniquekey/filename.mp4'))
                ->addRule('required');
        }
...
CUT
    ...
    public
        function _setConnector($connector)
    {
        $this->_connector = $connector;
    }

    public
        function getItems($path, array & $actions)
    {
        $items = array();
        if ($path == '')
        {
    ...
    public
        function getUrl(Am_Storage_File $file, $expTime)
    {
    ...
    public
        function action(array $query, $path, $url, Am_Request $request, Zend_Controller_Response_Http $response)
    {
        switch ($query['action'])
        {
            case 'refresh':
...
class SelectelAPI
{
    ...
    const
        STORAGE_URL_KEY = 'selectel_storage_url';
    const
        STORAGE_AUTH_TOKEN_KEY = 'selectel_auth_token_key';
     ...
     */
    protected
        function __request($uri, $method = Am_HttpRequest::METHOD_GET, $headers = array())
    {
    ...
    protected
        function __getXML($uri, $method = Am_HttpRequest::METHOD_GET, $headers = array())
    {
    ...
    protected
        function setStoredValue($key, $value, $timeout = 86400)
    {
