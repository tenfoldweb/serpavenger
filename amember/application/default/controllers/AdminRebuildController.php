...
class Am_Core_Rebuild
{
    ...
    function doWork(& $context, Am_BatchProcessor $batch)
    {
        ...
        if (!strlen($context))
        {
            $changed = Am_Di::getInstance()->userTable->checkAllSubscriptionsFindChanged($pageCount);
...
$(function(){
    ...
    function onDataReceived(data)
    {
            ...
            $("#process").find('.process-item-current').removeClass('process-item-current');
            $("#process").append('<div class="process-item process-item-current">' + data + "</div>");
            if (match = data.match(/$CONTINUE\((.+)\)$/))
            {
                ...
                if (data.match(/{$DONE}$/)) {
                    ...
                    $("#process").append('<div class="process-item process-item-error">Incorrect response received. Stopped!</div>');
                }
            }
            $("#process").scrollTop(($("#process")[0].scrollHeight - $("#process").height()));
    }
    ...
    $(".rebuild-button").click(function(){
        ...
        btn.val(btn.val().replace(" $DONE", "")+"…");
        $("#process").find('.process-item-current').removeClass('process-item-current');
        $("#process").append('<div class="process-item process-item-current">' + btn.val() + " $BEGIN</div>");
        $("#process").scrollTop(($("#process")[0].scrollHeight - $("#process").height()));
