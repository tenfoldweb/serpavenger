...
class Am_Newsletter_Plugin_Icontact extends Am_Newsletter_Plugin
{
    ...
    {
        $form->addText('user', array('size' => 60))
            ->setLabel(array('User name of icontact account', ''))
            ->addRule('required');

        $form->addText('apiappid', array('size' => 60))
            ->setLabel(array('API AppId', ''))
            ->addRule('required');

        $form->addText('apipass', array('size' => 60))
            ->setLabel(array('API Pass', ''))
            ->addRule('required');

        $form->addText('accountid', array('size' => 60))
            ->setLabel(array('Your account ID', ''))
            ->addRule('required')
            ->addRule('regex', ___('Digits only please'), '/^[0-9]+$/');

        $form->addText('clientfolderid', array('size' => 60))
            ->setLabel(array('Your client folder ID', ''))
            ->addRule('required')
            ->addRule('regex', ___('Digits only please'), '/^[0-9]+$/');
