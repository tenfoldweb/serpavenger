function _renderSiteAge(base_url,type){
    ...
    //alert(dataString);
    var u1= 'http://serpavenger.com/serp_avenger/';
    $.ajax({
        type: 'post',
...
function _renderSiteLongTermPageOneRank(base_url){
    ...
    $.ajax({
        type: 'post',
        url: u1 + 'renderanalysis/sitelongtermpageonerank',
        data: dataString,
        beforesend: function(){
...
function _renderSiteOnPageElement(base_url){
    ...
    var dataString  = 'campaign_list=' + encodeURIComponent(campaign_list) + '&campaign_server_engine=' + encodeURIComponent(campaign_server_engine);
            ...
            var u1= 'http://serpavenger.com/serp_avenger/index.php/';
    $.ajax({
        type: 'post',
...
function _renderSiteLinkElement(base_url){
    ...
    var base_url = 'http://serpavenger.com/serp_avenger/';
    $.ajax({
        type: 'post',
...
function _renderLongTermSite(base_url){
    ...
    $.ajax({
        ...
        success: function(data){
           ...
           var str ="";
           str +='<div class="serpp-info">'+ data.percent_data +'% Ranking for 60+ Days </div>';
...
function _renderSiteTopTenResults(base_url) {
            var campaign_list               = $('#campaign_list').val();
            var campaign_server_engine      = $('#campaign_server_engine').val();
            var keyword_value               = $('#keyword_value').val();
            ...
            var dataString  = 'campaign_list=' + encodeURIComponent(campaign_list) + '&campaign_server_engine=' + encodeURIComponent(campaign_server_engine);
             //alert(dataString);
             var u1= 'http://serpavenger.com/serp_avenger/index.php';
             var baeul= 'http://serpavenger.com/serp_avenger/';
            $.ajax({
                type: 'post',
                url: u1 +  'renderanalysis/sitetoptenresults',
                data: dataString,
                //dataType: 'json',
                beforesend: function(){
                     $('#contain-top-ten-results').hide();
                     $('#loader-top-ten-results').show();
                },
                success: function(data){
                    //alert(data);
                    var html = '';
                    ...
                    $.each($.parseJSON(data), function(idx, obj)
                     {
                        //alert(obj.url);
                        html  += '<li><div class="arrow_box"><a href="#" class="" onclick="DisData('+obj.id+','+i+');"><img src="http://serpavenger.com/serp_avenger/images/'+obj.campaign_murl_thumb+'"  id="image1"  width="74" height="56" alt=""><span class="ybg">Top '+i+'</span><div class=" clearfix"></div><small>'+obj.campaign_main_page_url+'</small></a></div></li>'; 
                        i = i+1
                      });

                     $( ".vertical-carousel-list" ).append(html); 
                }
            });
}
...
function _setDynmicData(base_url,type){
   ...
   _renderSiteAnalysisComparison(base_url);
    ...
    _renderSiteTopTenResults(base_url);
}
