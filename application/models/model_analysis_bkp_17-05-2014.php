...
*/				
//print_r($this->get_recovery_site($campaign_list, $campaign_server_engine));
//$this->get_long_term_site($campaign_list, $campaign_server_engine);
//die;
				...
				foreach($query->result_array() as $row=>$data){
					...
					if(is_array($data) && count($data) > 0){
						...
						if($query2->num_rows() > 0){
							...
							$percent_10_num_today	= ($avg_num_rows_today/10)*100;
						}
						//echo $data['date_added'] . ' | ' . date("Y-m-d") . '<br>';
						if($data['date_added'] == date("Y-m-d")){
							$rec['percent_10_num']	= $percent_10_num_today;
...
$crawl_server="TC.isCrawlByBing = 1";
		...
		$date_WC ='AND CG.date_added <= "'.$this->sql_current_date.'"';
		$date_WC .=' AND CG.date_added >= "'.date("Y-m-d", strtotime( "-".$this->graph_data_limit." days", strtotime($this->sql_current_date))).'"';
		$sql	= "SELECT MAX(domain_kw_ratio) AS maxKW,
				  MIN(domain_kw_ratio) AS minKW,
				  FORMAT(AVG(domain_kw_ratio), 2) AS avgKW,
