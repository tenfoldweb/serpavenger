...
class Scrapper_model extends CI_Model {
 public function __construct()
 {
  parent::__construct();
	...
	*/
 	function get_domains($ids, $orderby='', $order="", $condition=""){
		...
		if($orderby !='')
		{
			if($orderby=="domainip")
			{
				$condition ="and domainip  not in (
				SELECT domainip  FROM domains where status = 1
				group by domainip  
				having count(*) >1 )
				union all
				SELECT domainid, domainname, network_id, type FROM domains WHERE status = 1 and network_id IN ( $ids ) and domainip  in (
				SELECT domainip  FROM domains where status = 1
				group by domainip  
				having count(*) >1 )";
			}
			...
			$select_sql = "SELECT domainid, domainname, network_id, type FROM domains WHERE status = 1 and network_id IN ( $ids ) ".$condition;
			$query = $this->db->query($select_sql);
			//$num_rows=$query->num_rows();	
			//return $num_rows;
			if($query->num_rows() > 0)
			  $res = $query->result_array();
                          $domain_name=$res['domainname'];
	 ...
	 function domainpost_is_exist($post){
		 
		    $select_sql="SELECT * FROM campaign_posts WHERE domain_id=".$post['domain_id']." AND ((anchor1='".$post['anchor1']."' AND link1='".$post['link1']."') OR (anchor1='".$post['anchor2']."' AND link1='".$post['link2']."') OR (anchor1='".$post['anchor3']."' AND link1='".$post['link3']."') OR (anchor2='".$post['anchor1']."' AND link2='".$post['link1']."') OR (anchor2='".$post['anchor2']."' AND link2='".$post['link2']."') OR (anchor2='".$post['anchor3']."' AND link2='".$post['link3']."') OR (anchor3='".$post['anchor1']."' AND link3='".$post['link1']."') OR (anchor3='".$post['anchor2']."' AND link3='".$post['link2']."') OR (anchor3='".$post['anchor3']."' AND link3='".$post['link3']."'))";
			$query = $this->db->query($select_sql);
	...
	function word_format($position,$content){
		...
		function key_replace($anchor, $link, $content, $syms_all, $settings = ''){
			...
			if(substr($content, $pos-1, 1) == " " || substr($content, $pos+strlen($find_words), 1) == " ")
			{
					...
					if(!empty($myfind[$position])){	
					   if ($pos !== false) {
						...
						$content=str_replace($find_words.$myfind[$position],$content_new,$content);	
						   ...
						   $content = str_replace($content,$content.' <a href="'.$link.'" title="">'.$anchor.'</a> ',$content);
						   //$content = $content.' <a href="'.$link.'" title="">'.$anchor.'</a> ';
					   }
					}
				}
			...
			for($i=0;$i<=$perpost;$i++){
			 $comment_num_rand = rand(2,$nums_of_comments);
               ...
               //echo '1'.$data[$c];
				   ...
				   $comnts = preg_replace("/[^0-9a-zA-Z.?,;'!\-%\s]/", "", $all_comments[$comment_num_rand]);
				   
				   $comment_count++;
                   $sql = "INSERT INTO post_comments SET
			       post_id	    = ".$post_id.",
			       comments		= '".addslashes($comnts)."',
			       status	= 'publish',
	 ...
	 public function user_count($user_id='',$status='') {

	    //$this->db->limit($limit, $start);
	    $this->db->select('*');	
	    $this->db->select("COUNT(campaign_posts.domain_id) AS count");
		$this->db->from('campaign_posts');

		$this->db->join('campaign','campaign_posts.campaign_id = campaign.campaign_id');
			
		if($user_id){ 
			 $this->db->where('campaign_posts.user_id = campaign.user_id');	
		}
		if($status){
			$this->db->where('post_status', $status);
		}
		
		$this->db->group_by('campaign_posts.campaign_id');
		$result = $this->db->get();
	    $num = $result->num_rows();
		
    // return $this->db->count_all("campaign_posts");
		return $num;
    }
	   
	 function get_active_posts($user_id='',$status='',$limit, $start)
	    {
	    	//print($user_id); print($status); print($id); exit;
		//$this->db->select(COUNT( 'campaign_posts'.'domain_id') as 'test' );
	    $this->db->limit($limit, $start);
	    $this->db->select('*');	
	    $this->db->select("COUNT(campaign_posts.domain_id) AS count");
		$this->db->from('campaign_posts');
	...
	public function update_keywords($campaignid, $userid, $topics, $keywords, $synonyms)
	{
		...
		$this->db->insert('serp_users_campaign_keywords',$update_data);
	}
		...
		public function getUsersCampaigns($users_id){
			...
			if($query->num_rows() > 0){
				...
				for($i=0; $i<count($rec); $i++){
					$sql	= '';
					$query	= '';
				$sql	= "SELECT * FROM serp_users_campaign_keywords AS CK, serp_users_campaign_detail AS UC WHERE CK.campaign_id = UC.campaign_id AND UC.c_id = '".$rec[$i]['campaign_id']."' group by CK.keyword";
					$query	= $this->db->query($sql);
					if($query->num_rows() > 0){
    ...
    {
        $query = $this->db->where('campaign_id', $id);
        $query = $this->db->limit(1,0);
        $query = $this->db->delete('campaign');
	...
	public function fetch_permission_package_id($name){
		$this->db->select('product_id');	
		$this->db->from('am_product');
		$this->db->where('title', $name);
		$query = $this->db->get();
		$result = $query->result();
		return $result[0]->product_id;
	}
	public function count_number_submission($id){
		$this->db->select('*');	
		$this->db->from('serp_submission_counter');
		$this->db->where('user_id', $id);
		$query = $this->db->get();
		$result = $query->result();
		return $result;
	}
	public function counter_insert($data){
		$this->db->insert('serp_submission_counter', $data); 
		return true;
	}
	public function counter_update($data){
		$user_id = $data['user_id'];
		unset($data['user_id']);
			$this->db->where('user_id', $user_id);
			$this->db->update('serp_submission_counter', $data); 
		return true;
	}
	public function user_added_permission($data){
		
		$this->db->delete("serp_added_user_permission", array('user_id' => $data['user_id'], 'package_id' => $data['package_id']));
		
		$this->db->insert('serp_added_user_permission', $data); 
		return true;
	}

function param_insert( $data ){
		$sql = "INSERT INTO `serp_post_param` (`id`, `file_name`, `queue_time`) VALUES (NULL, '".$data."', CURRENT_TIMESTAMP)"; 
		$query	= $this->db->query($sql);
		return true;
	 }
function param_insert_a( $data ){
		$sql = "INSERT INTO `serp_post_param_a` (`id`, `file_name`, `queue_time`) VALUES (NULL, '".$data."', CURRENT_TIMESTAMP)"; 
		$query	= $this->db->query($sql);
		return true;
	 }
	function param_update( $id ){
		$data = array(
               'status' => 'p',
            );
		$this->db->where('id', $id);
		$this->db->update('serp_post_param', $data); 
		
		return true;
	 }

function param_update_a( $id ){
		$data = array(
               'status' => 'p',
            );
		$this->db->where('id', $id);
		$this->db->update('serp_post_param_a', $data); 
		
		return true;
	 }

          function queue_data_a($status=''){
		$this->db->select('*');
		$this->db->from('serp_post_param_a');		
		if($status!='')
	...
	function queue_data($status=''){
		$this->db->select('*');
		$this->db->from('serp_post_param');		
		if($status!='')
		$this->db->where('status', $status);
		else
		$this->db->where('status', 'a');
	 ...
	 public function add_custom_permissions($permission_name, $permission_cost)
	 {
		  $add_package = false;
		  $package_id = "";
		  $query = $this->db->select("*")->from("am_product")->get();
		  ...
		  {  
			  ...
			  $amember_product['title'] = $permission_name;
			  ...
			  $amember_billing_plan['first_price'] = $permission_cost;
			  $amember_billing_plan['first_period'] = '1m';
			  $amember_billing_plan['rebill_times'] = '99999';
			  $amember_billing_plan['second_price'] = $permission_cost;
 ...
 public function tildescpopmodal_update($postdata)
    {
		$sqlkey	= "UPDATE `campaign_posts` SET
		  post_title			= '".$postdata['ptitle']."',
		  post_content	= '".$postdata['pcontent']."' ,
		  post_modified = now() 		 
		WHERE 	 
		  ID ='".$postdata['pid']."' ";
		$s = $this->db->query($sqlkey); 
	 return $s;
     }
