...
class Model_analysistest extends CI_Model
{   
	var $tblUsersCampaignKeywords = 'serp_users_campaign_keywords_test';
	var $tblCampaigns		= 'serp_users_campaign_detail_test';
	var $tblCrawledURLDataGoogle	= 'serp_google_crawl_data_test';
...
function get_recovery_site($campaign_list, $campaign_server_engine){
		...
		switch($campaign_server_engine){
			case "google" :
				$table="serp_google_crawl_data_test";
...
$crawl_server="TC.isCrawlByBing = 1";
		...
		$WC="";
				...
				if(!empty($campaign_list)){
					...
					$cam_sql  =  "SELECT UCK.keyword_id FROM serp_users_campaign_keywords_test AS UCK
						  WHERE UCK.campaign_id IN (
								SELECT ucd.campaign_id
								FROM serp_users_campaign_detail_test ucd,
...
function get_long_term_site($campaign_list, $campaign_server_engine){
		...
		switch($campaign_server_engine){
			case "google" :
				$crawl_table="serp_google_crawl_data_test";
$crawl_server="TC.isCrawlByGoogle = 1";
				break;
			case "yahoo" :
...
public function renderUsersDomainAge($campaign_list, $campaign_server_engine,$site_type){
		...
		switch($campaign_server_engine){
			case "Google" :
				$this->tblCrawledURLDataGoogle="serp_google_crawl_data_test";
				$crawl_server="TC.isCrawlByGoogle = 1";
				break;
			case "Yahoo" :
	...
	public function renderUsersLongTermPageOneRank(){
		 ...
		 //Edited by BEAS - Fetching logged in user name
   ...
   public function rendertoptenresults($campaign_list, $campaign_server_engine,$keyword){
	...
	//}
			...
			public function count_keyword_by_user($user_id) {
				$this->db->select('*');
				$this->db->from('serp_users_campaign_keywords_test');
				$this->db->where('users_id', $user_id);
				$this->db->where('analyzed', 'True');
				$result_count_keyword_by_user = $this->db->get();
				
				return $result_count_keyword_by_user = $result_count_keyword_by_user->result();
			}

function showcontentdata($id){
		    //echo "test11"; print_r ($_REQUEST);
			    $user_id		= $this->session->userdata("LOGIN_USER");
					$rec			= FALSE;
					$record			= FALSE;

              //$cal_date=date("Y-m-d", strtotime("-0 days", strtotime($this->sql_current_date)));

					// $sql	= "SELECT CG.*,TC.*,DATEDIFF( NOW(), CG.domain_creation_date ) AS age, CASE
				 //    WHEN DATEDIFF( NOW(), CG.domain_creation_date ) >= 365 THEN 'old'
				 //    WHEN (DATEDIFF(NOW(), CG.domain_creation_date) < 365 AND DATEDIFF(NOW(), CG.domain_creation_date) > 180) THEN 'young'
				 //    WHEN DATEDIFF(NOW(), CG.domain_creation_date) <= 180 THEN 'new'
				 //    END AS ageType,
				 //    CG.date_added
					// 	 FROM serp_google_crawl_data_test AS CG, serp_users_campaign_detail_test AS TC
					// 	 WHERE CG.campaign_id = TC.campaign_id
					// 	 AND CG.date_added = '".$cal_date."'
					// 	   AND CG.id = '".$id."'
