...
//						$sql= str_replace($camp_sql, "", $sql);
	...
	/*public function get_rendertoptenresults($campaign_list, $campaign_server_engine,$site_type){
	echo $usr 			= $this->session->userdata('current_user');
	$user_id		= $this->session->userdata("LOGIN_USER");
	$rec			= FALSE;
	$record			= FALSE;
	echo "sql".$sql	= "SELECT * FROM " . TABLE_USERS_CAMPAIGN_MASTER . " WHERE users_id = '".$users_id."' and campaign_main_keyword = '".$campaign_list."' ";
		$query	= $this->db->query($sql);
		if($query->num_rows() > 0){
		   // pr($query->result_array());
		    foreach($query->result_array() as $index=>$data){
	...
	/*public function rendertoptenresults1($campaign_list, $campaign_server_engine,$site_type){
			$usr 			= $this->session->userdata('current_user');
			$user_id		= $this->session->userdata("LOGIN_USER");
			$rec			= FALSE;
			$record			= FALSE;
