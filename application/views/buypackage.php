...
<?php /*if(isset($package) && count($package) > 0) {
	...
	//var proids = "<?//=$pckg['proids']; ?>";
	
	<?php //break; }} ?>
	
	/*var owned_packages = proids.split(',');
	
	if(owned_packages.length > 0)
	{
		for(var i=0; i<owned_packages.length; i++)
		{
			$('#pckg'+owned_packages[i]).attr('disabled',true);
			$('#msg'+owned_packages[i]).show();
		}
	}	  
	
	$('input[name=submit]').click(function(){

		if($('input[name=package_name]').is(':checked'))
		{
			var pckgdata = $('input[name=package_name]:checked').val();
			
			var package = pckgdata.split('|');

			$('input[name=item_number]').val(package[0]);
			$('input[name=item_name]').val(package[1]);
			$('input[name=a3]').val(package[2]);
			//$('input[name=discount_rate]').val(package[3]);
			
			$('#redirect_msg').show();
		}
		else
		{
		  alert('Select a package to continue');
		  return false;
		}
	}) */	
      ...
      <div class="package">
        ...
        <div class="usr_permission">
            ...
            <thead>
              ...
              <tr>
              <td colspan="2">
              <!--<form name="_xclick" id="buy" action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">-->
              <form name="_xclick" id="buy" action="https://sandbox.paypal.com/cgi-bin/webscr" method="post" target="_top">
                ...
                <input type="hidden" name="return" value="http://serpavenger.com/serp_avenger/buypackage?stat=1">
