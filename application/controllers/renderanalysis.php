...
class Renderanalysis extends CI_Controller {
	...
	public function siteanalysiscomparison(){
		...
		$renderList	= $this->model_analysis->renderanalysiscomparison($campaign_list, $campaign_server_engine,$keyword_value);
	...
	public function renderserpmeter()
	{
		...
		*/
		$data_top10_today = $this->model_analysis->renderserpmeter($campaign_list, $campaign_server_engine, $currDate);
		$data_top10_prevDate = $this->model_analysis->renderserpmeter($campaign_list, $campaign_server_engine, $prevDate);
		$data_top10_prev2date = $this->model_analysis->renderserpmeter($campaign_list, $campaign_server_engine, $before2Date);
		$data_top10_prev7date = $this->model_analysis->renderserpmeter($campaign_list, $campaign_server_engine, $before7Date);
		$data_top10_prev30date = $this->model_analysis->renderserpmeter($campaign_list, $campaign_server_engine, $before30Date);
		$diff_yesterday = 0;
		for($i= 0;$i<10;$i++)
		{
	...
	function sitetoptenresults(){
			//$this->check_login();
			$campaign_list			= $this->input->get_post('campaign_list');
			$campaign_server_engine		= $this->input->get_post('campaign_server_engine');
			$keyword_value			= $this->input->get_post('keyword_value');
			$renderList	= $this->model_analysis->rendertoptenresults($campaign_list, $campaign_server_engine,$keyword_value);
