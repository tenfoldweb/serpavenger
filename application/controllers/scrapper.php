...
class Scrapper extends CI_Controller {
	...
	public function index()
	{
		...
		if((isset($_REQUEST['tx']) && $_REQUEST['tx']!='') && (isset($_REQUEST['st']) && $_REQUEST['st']=='Completed')){
			...
			$get_packages[$package_index]['submitter_permission'] = 1;
		...
		$data['networks'] = $this->network_model->get_network('',$session['user_id']);
		...
		$data['active_campaignList'] =  $this->model_basic->getActiveCampaignList($users_id);	
	...
	function form($campaign_id = "")
	{
		...
		// print_r($session);
		 ...
		 $get_packages = $this->userlogin_model->get_packages($user_id);
		...
		if($spintype == 'manually')
		{
			...
			$post_content1 = $this->scrapper_model->spin($this->input->post('post_content'));
		}
		 if($spintype == 'smart_content')
			  {		
				$generic_title = $this->input->post('choose_generic_title');
				
				if($generic_title == "custom")
				  $this->form_validation->set_rules('generic_title', 'Enter General Title', 'required');
				else
				{			
				...
				if($this->form_validation->run() == FALSE)
				{
					...
					if($generic_title == "custom")
					 $errmsg = "Please Enter Generic Title ";
					else
					 $errmsg = "Please Enter General Topics <br> Provide Keywords <br> Provide Synonyms";
					
					$this->session->set_flashdata('message', '<div class="notification note-error">
					<a title="Close notification" class="close" href="#">close</a>
					<p>For SERP Avenger Smart Content - <br><br> '.$errmsg.'</p>
				</div> ');
		...
		//Submission counter check
		$total_max_scrap = 0;
		$total_max_article = 0;
		
		foreach($get_packages as $row){
				$total_max_scrap += $row['max_scraped_runs_no'];
				$total_max_article += $row['max_article_no'];
		}	

		$count_number_submission = $this->scrapper_model->count_number_submission($session['user_id']);

		if(!empty($count_number_submission) && count($count_number_submission)>0){
			if(($spintype == 'manually' && $total_max_article > $count_number_submission[0]->article_count) && $count_number_submission[0]->month == date("m")){
				
				
					$data_counter = array(
								'user_id' => $session['user_id'], 
								'article_count' => $count_number_submission[0]->article_count+1
				);
				$counter_update = $this->scrapper_model->counter_update($data_counter);
			}elseif($spintype == 'smart_content' && $total_max_scrap>$count_number_submission[0]->scrap_count && $count_number_submission[0]->month == date("m")){
				$data_counter = array(
								'user_id' => $session['user_id'], 
								'scrap_count' => $count_number_submission[0]->scrap_count+1
			...
			$counter_update = $this->scrapper_model->counter_update($data_counter);
				...
				if($spintype == 'smart_content')
					$this->session->set_flashdata('message', '<div class="notification note-error">
										<a title="Close notification" class="close" href="#">Close</a>
										<p>The number of scrap submission for this month is over.</p>
									</div>');
				else
				$this->session->set_flashdata('message', '<div class="notification note-error">
										<a title="Close notification" class="close" href="#">close</a>
										<p>The number of article submission for this month is over.</p>
									</div>');
			...
			if($spintype == 'manually'){
				...
				$counter_update = $this->scrapper_model->counter_insert($data_counter);
				}else{
					$data_counter = array(
									'user_id' => $session['user_id'], 
									'article_count' => 0, 
			...
			$unique_comments = $this->input->post('unique_comments');
				if(!empty($_FILES) && $unique_comments==''){								
				  ...
				  //  file is empty and unique_comments blank. below else part
		...
		if($spintype == 'manually' && $unique_comments!=''){
                       // This code is for store the post data in text file that will be used queue process
			$actual_filename  = 'filename_'.time().'.txt';
			array_push($_POST,$all_comments);
				
			$filename = './assets/uploads/'.$actual_filename;
			file_put_contents($filename, serialize($_POST));
			$post_param = $this->scrapper_model->param_insert_a($actual_filename);
			
			$this->session->set_flashdata('message', '<div class="notification note-success">
						<a title="Close notification" class="close" href="#">close</a>
						<p>Your form has been submitted and added to queue. !</p>
					</div> ');

				redirect('scrapper');	

		}elseif($spintype != 'manually'){
			// This code is for store the post data in text file that will be used queue process
			$actual_filename  = 'filename_'.time().'.txt';
			array_push($_POST,$all_comments);
			...
			$post_param = $this->scrapper_model->param_insert($actual_filename);
			//echo "yed";
            //exit($post_param);
			$this->session->set_flashdata('message', '<div class="notification note-success">
						<a title="Close notification" class="close" href="#">close</a>
						<p class="msg error">Your form has been submitted and added to queue. !</p>
					</div> ');
			...
			$special_chars = array("?", "[", "]", "/", "\\", "=", "<", ">", ":", ";", ",", "'", "\"", "&", "$", "#", "*", "(", ")", "|", "~", "`", "!", "{", "}");
			  ...
			  if($spintype == 'smart_content')
			  {
				...
				if($this->form_validation->run() == FALSE)
				{
					...
					$this->session->set_flashdata('message', '<div class="notification note-error">
					<a title="Close notification" class="close" href="#">close</a>
					<p>For SERP Avenger Smart Content - '.$errmsg.'<br><br> </p>
				</div> ');
				...
				else
				{
					...
					if($generic_title == "custom")
					  $get_generic_title = $this->input->post('generic_title');
					else
					{
					...
					$campaign_id = $this->input->post('campaign');    //campaign id
					...
					{
						$mode = rand(0,1);
						$stat = $this->syscon($searchmode[$mode], $get_generic_title, $content_keywords, $content_synonyms, $content_topics, $user_id, $campaign_id);
	 ...
	 $selected_domains = explode(',',$this->input->post('selected_domains'));
	 
	 for($k=0;$k < count($selected_domains);$k++)
	 {

     for($domain=0;$domain < count($res);$domain++)
	 {
		 if(trim(strtolower($selected_domains[$k])) == trim(strtolower($res[$domain]['domainname']))) {
	  ...
	  /* if($res[$domain]['type']=='Traditional Blog' || $res[$domain]['type']=='Blog+')
	   { */  
			...
			$this->scrapper_model->save($save);
			
			$campaign_id = $save['campaign'];
						...
						if(isset($newtot) && count($newtot) > 0)
						{
							while(1)
							{
							     if(isset($newtot[$i]))
								 {
								   ...
								   $post_content = preg_replace("/[^0-9a-zA-Z.?,;'!\-%\s]/", "", $post_content);
								 }
					...
					/******************unique domain*********************/   
					 ...
					 if($posted_domain){
						 ...
						 $post_id=$this->scrapper_model->post_save($save_post);  
						  if($post_id && $is_comment=='on' &&  $res[$domain]['cmstype']=='WP Blog +'){   // checking if SERP Avenger Comment Seeding is on
	...
	public function filter_content($content)
	{
		...
		$content = preg_replace("#[â€™|â€|™|©|®|\#|=|:]*#",'',$content);
                ...
                $content = preg_replace("#[’|\E2\80|\99|\A9|\AE|\#|=|:]*#",'',$content);
 ...
 public function get_random($record, $key)
 {
	...
	public function syscon($action, $generic_title='', $keywords, $synonyms, $topics, $user_id, $campaign_id){
		...
		if($generic_title != "")
		{
			$status = $this->syscon_get_crawl($action, $generic_title, '', $user_id, $campaign_id);
		}
		else
		{
			...
			foreach($sentence['keyword'] as $keyword_crawl){
				...
				$status = $this->syscon_get_crawl($action, $searchstring, $keyword_crawl['word'], $user_id, $campaign_id);
			}
	    }
	...
	public function syscon_get_crawl($action, $searchstring, $keyword='', $user_id, $campaign_id)
	{
		$proxy_list	= $this->scrapper_model->getValues_conditions(TABLE_PROXIES, '*', '', 'status = 1');
	...
	public function comment_syscon($keywords, $campaign_id){
		...
		if($action == 'yahoo'){
            ...
            $email_pattern = '/[a-z0-9!#$%&\'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&\'*+\/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/';
				...
				foreach($search_crawl_data as $search_crawl_data_val){
					...
					//$output['serp_google_crawl_data_id'] =  $search_crawl_data_val['id'];
					$output['campaign_id'] =  $search_crawl_data_val['campaign_id'];
					$output['users_id'] =  $session['user_id'];
					//$output['crawlby'] =  $search_crawl_data_val['crawlby'];
					$output['url'] =  $search_crawl_data_val['url'];
					$output['proxy'] =  $search_crawl_data_val['proxy'];
					$output['keyword'] =  $search_crawl_data_val['keyword'];
...
public function user_queue ($campaign_id = "")
	{
	$session = $this->session->userdata('user_data');
		...
		$link3    = '';
		        ...
		        if(isset($_GET['queue'])&& $_GET['queue']!=''){
                          ...
                          $queue_data = $this->scrapper_model->queue_data_a();

			}elseif(isset($_GET['status']) && $_GET['status']!=''){
				$queue_data = $this->scrapper_model->queue_data($_GET['status']);
			}else{
				$queue_data = $this->scrapper_model->queue_data();
			}
			
			
if(!empty($queue_data)){

		$this->scrapper_model->param_update($queue_data[0]->id);
		$file_data = file_get_contents('./assets/uploads/'.$queue_data[0]->file_name);
		$contents = unserialize($file_data);  
		//print_r($contents); die;
		$spintype = $contents['spin_type'][0];
		$is_comment = $contents['serp_comment'];
		
		if($contents['unique_comments']){
		$all_comments=array();
		 //  file is empty and unique_comments blank. below else part
				      // Here will be unique part that have found from yahoo answers 
				      $keywords = $contents['keywords'];
				      $spintaxreturn = $this->comment_syscon($keywords, $contents['campaign']);
					  $arr=array();
							 $nn=explode('# B #',$spintaxreturn);
							for($i=1;$i<count($nn);$i++){
									for($j=1;$j<=5;$j++){			
			...
			$all_comments = $contents[0];
		}
		$post_title = $this->scrapper_model->spin($contents['post_title']);
		$post_content1 = $this->scrapper_model->spin($contents['post_content']);
		
			$user_id = $session['user_id'];
            
			if($contents['drip_rate']=="Custom Range"){
				$drip_rate = $contents['num_post']."/".$contents['postings'];
				
			}else{
				$drip_rate = $contents['drip_rate'];
			}
			
			$networks = implode(",", $contents['networks']);
			
			$favor_preference = $contents['favor_preference'];
			  if($favor_preference == 'Unique IP First'){
				  $orderby = 'domainip'; $order = 'DESC';
			  }else if($favor_preference == 'Highest Pagerank First'){
				  $orderby = 'pagerank'; $order = 'DESC';
			...
			$is_format = $contents['serp_format']; 
			$nums=$contents['submission_num'];
			$special_chars = array("?", "[", "]", "/", "\\", "=", "<", ">", ":", ";", ",", "'", "\"", "&", "$", "#", "*", "(", ")", "|", "~", "`", "!", "{", "}");
			  ...
			  if($spintype == 'smart_content')
			  {
				  ...
				  $content_synonyms = "";
					...
					if(isset($contents['generic_title']) && $contents['generic_title'] != "")
					  $generic_title = $contents['generic_title'];
					else
					{
						$content_topics = $contents['smart_content_topics'];
						$content_keywords = $contents['smart_content_keywords'];
						$content_synonyms = $contents['smart_content_synonyms'];
					}
					
					$campaign_id = $contents['campaign'];
					...
					{
						$mode = rand(0,1);
						$stat = $this->syscon($searchmode[$mode], $generic_title, $content_keywords, $content_synonyms, $content_topics, $user_id, $campaign_id);
     ...
     $selected_domains = explode(',',$contents['selected_domains']);
     ...
     for($domain=0;$domain < count($res);$domain++)
	 {
	  ...
	  /* if($res[$domain]['type']=='Traditional Blog' || $res[$domain]['type']=='Blog+')
	   {*/
			if($contents['schedule']=="later"){
				$schedule = $contents['start_date'];
				$date = $contents['start_date'];
			}else if($contents['schedule']=="now"){
				$schedule = $contents['schedule'];
				$date = date("Y-m-d H:i:s");
			}	
			if($contents['drip_rate']=="Custom Range"){
				
				$date = strtotime(date("Y-m-d H:i:s", strtotime($date)) . " +1 ".$contents['postings']);
				$date = date("Y-m-d H:i:s",$date);
			
			}else if($contents['drip_rate']=="24 Hours"){
				$hours=rand(1,20);	
				$date = strtotime(date("Y-m-d H:i:s", strtotime($date)) . " +".$hours." hour");
				$date = date("Y-m-d H:i:s",$date);
				
			}else if($contents['drip_rate']=="Viral Linking"){
				$days=rand(1,7);	
				$date = strtotime(date("Y-m-d H:i:s", strtotime($date)) . " +".$days." days");
				$date = date("Y-m-d H:i:s",$date);
				
			}else if($contents['drip_rate']=="Mini Spikes"){
				$days=rand(7,10);	
				$date = strtotime(date("Y-m-d H:i:s", strtotime($date)) . " +".$days." days");
				$date = date("Y-m-d H:i:s",$date);
			} 
			...
			$save['networks']	   = $networks;
			$save['domain_id']	  = $res[$domain]['domainid'];
			$save['project_name']   = $contents['project_name'];	
			$save['campaign']	   = $contents['campaign'];
			$save['spin_type']	  = $spintype;	
			$save['submission_num'] = $contents['submission_num'];
			$save['submission']     = $submission;
			$save['favor_preference']= $favor_preference;	
			$save['schedule']	   = $schedule;
			if(isset($contents['comment_seeding']))
			$save['comment_seeding']= $contents['comment_seeding'];
			else
			$save['comment_seeding']= "";
			$save['comment_file']   = $provalue;
			$save['drip_rate']      = $drip_rate;
			$save['create_date']	= date("Y-m-d H:i:s");
			$save['update_date']	= '';
			...
			if($campaign_id != ""){
				...
				for($i=1;$i<=$nums;$i++)
				{
					...
					if($is_format=='on'){   // checking if SERP Avenger  Professional Formatting is on					
					   ...
					   for($n1=0;$n1 < $contents['count_r'];$n1++){
						     $anc= $n1+1; 
							 $num_qty=$contents['qty'.$anc];
                             $min_qty=$contents['min_qty'.$anc];
		 				    //if($i<=$num_qlty)
                             if($i>=$min_qty && $i<=$num_qty){
							 $anchor1 = $this->scrapper_model->spin($contents['anchor'.$anc]);
							 $link1 = $this->scrapper_model->spin($contents['link'.$anc]);
							 
							 
							 
							 $settings = $contents['anchor_set'.$anc];
							 $syns=$contents['synonyms'];
								 if(!empty($syns)){
							 $syms_all = $syns[$n1];
							 if($anchor1!='' && $link1!='' && !empty($syms_all)){ 
						     	$post_content = $this->scrapper_model->key_replace($anchor1, $link1, $post_content, $syms_all, $settings);
								 ...
								 if($contents['link_identifier']=='on'){
									if($anchor1!='' && $link1!=''){
									$post_content = $this->scrapper_model->link_identifier($anchor1, $link1,$post_content, '%link1%');
									}
								 }
					   ...
					   for($n2=0;$n2 < $contents['count_rr'];$n2++){
						     $anc2= $n2+1; 
							 $num_qty2 = $contents['qty_rr'.$anc2];
                             $min_qty2 = $contents['min_qty_rr'.$anc2];
                             if($i >= $min_qty2 && $i <= $num_qty2){
							 $anchor2 = $this->scrapper_model->spin($contents['anchor_rr'.$anc2]);
							 $link2 = $this->scrapper_model->spin($contents['link_rr'.$anc2]);
							 $settings2 = $contents['anchor_set_rr'.$anc2];
							 
                             $syns2=$contents['synonyms_rr'];
								 if(!empty($syns2)){
							 $syms_all2 = $syns2[$n2];
							 if($anchor2!='' && $link2!='' && !empty($syms_all2)){ 
						     	$post_content = $this->scrapper_model->key_replace($anchor2, $link2, $post_content, $syms_all2, $settings2);
					   ...
					   for($n3=0;$n3 < $contents['count_rrr'];$n3++){
						     $anc3= $n3+1; 
							 $num_qty3 = $contents['qty_rrr'.$anc3];
                             $min_qty3 = $contents['min_qty_rrr'.$anc3];

                             if($i >= $min_qty3 && $i <= $num_qty3){

							 $anchor3 = $this->scrapper_model->spin($contents['anchor_rrr'.$anc3]);
							 $link3 = $this->scrapper_model->spin($contents['link_rrr'.$anc3]);
							 $settings3 = $contents['anchor_set_rrr'.$anc3];							 
							 $syns3=$contents['synonyms_rrr'];
								 if(!empty($syns3)){
							 $syms_all3 = $syns3[$n3];
								 if($anchor3!='' && $link3!='' && !empty($syms_all3)){ 
									$post_content = $this->scrapper_model->key_replace($anchor3, $link3, $post_content, $syms_all3, $settings3);
...
if(isset($_GET['queue'])&& $_GET['queue']!=''){
			$serp_post_param = 'serp_post_param_a';
			$this->db->delete($serp_post_param, array('id' => $queue_data[0]->id)); 
}else{
$serp_post_param = 'serp_post_param';
			$this->db->delete($serp_post_param, array('id' => $queue_data[0]->id));
}

			
			unlink('./assets/uploads/'.$queue_data[0]->file_name);
...
public function queue_list(){
		...
		$queue_data = $this->scrapper_model->queue_data_all();
		if(!empty($queue_data)){
		$i=0;
		foreach($queue_data as $data){
			echo ++$i." . &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$data->file_name." &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$data->queue_time." . &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ".$data->status."  <br>";
		}
				}else{
				echo "No queue data found.";
				}
		}
...
public function add_user_permissions()
	...
	public function demo_form ($campaign_id = "")
	{
	$session = $this->session->userdata('user_data');
			...
			$special_chars = array("?", "[", "]", "/", "\\", "=", "<", ">", ":", ";", ",", "'", "\"", "&", "$", "#", "*", "(", ")", "|", "~", "`", "!", "{", "}");
			  ...
			  if($spintype == 'smart_content')
			  {
				...
				else
				{
					...
					{
						$mode = rand(0,1);
						$stat = $this->syscon($searchmode[$mode], $content_keywords, $content_synonyms, $content_topics, $user_id, $campaign_id);
