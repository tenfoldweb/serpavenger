...
class bot {
    ...
    public function __construct() {
        $this->agent = "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/534.14 (KHTML, like Gecko) Chrome/9.0.600.0 Safari/534.14";//Agent
    ...
    public function initSpider( $bot, $keyword, $proxy, $location, $page, $delay,$proxyUser,$proxyLocation ) {
        $url =  $html = '';
        $json = array();
        ...
        if ( $bot == 'gr' ) {
            if ( $page == 1 )
                $url = 'https://www.google.'.$location.'/search?q='.str_replace( ' ', '+', $keyword ).'&ie=UTF-8&num=100&tbs=li:1'; //Google Rank
            else
                $url = 'https://www.google.'.$location.'/search?q='.str_replace( ' ', '+', $keyword ).'&ie=UTF-8&num=100&start='.( ( $page-1 )*100 ).'&tbs=li:1'; //Google Rank
        }elseif ( $bot == 'bing' ) {
            if ( $page == 1 )
        ...
        /* Debug Entry */
        /*echo '<p>Proxy : '.$proxy.'<br />';
        echo 'Proxy Location : '.$proxyLocation.'<br />';
        echo 'Google URL : '.$url.'<br /></p>';*/

        /*Do crawl*/
        if ( $bot == 'gb' || $bot == 'gr' )
            $html = str_get_html( crawl_simple( $this->reffer_google, $url, $this->agent, $proxy ,$proxyUser) );//Google Rank and Blog
        elseif ( $bot == 'bing' )
            $html = str_get_html( crawl_simple( $this->reffer_bing_rank, $url, $this->agent, $proxy ,$proxyUser) );//Bing Rank
        elseif ( $bot == 'yahoo' )
            $html = str_get_html( crawl_simple( $this->reffer_yahoo_rank, $url, $this->agent, $proxy,$proxyUser ) );//Yahoo Rank
        ...
        if ( $bot == 'gb' || $bot == 'gr' ) {
            ...
            foreach ( $html->find( 'li[class=g]' ) as $index => $li ) {
                ...
                $json[$index]['url'] = urldecode(str_replace( '/url?q=', '', $li->find( 'a', 0 )->href ));
            ...
            foreach ( $html->find( 'ul[id=yan-questions]', 0 )->find( 'li' ) as $index => $li ) {
                ...
                $html_answer = str_get_html( crawl_simple( $this->reffer_yahoo, trim( $this->reffer_yahoo.$li->find( 'h3', 0 )->find( 'a', 0 )->href ), $this->agent, $proxy,$proxyUser ) );//Yahoo Answer
