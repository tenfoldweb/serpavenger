...
function get_network_domain_from_db($id)
	{	
		...
		$id = $_POST['id'];
		$this->load->model('model_mypannel');
		$data =$this->model_mypannel->get_network_list_data_all($id,17,0); 
		//print_r($data); 
	//	die;
		$i=0;
 		echo "<ul>";
 		foreach ($data as $key) {
			...
			if($i <6)
 			{ 	
				...
				if($key->thumb!='')
 				echo '<li><img src="http://www.serpavenger.com/serp_avenger/images/screenshots/'.$key->thumb.'.jpg" width="120" height="90" ><a href="http://'.$key->domainname.'" target="_blank" ><span>'.$key->domainname.'</span></a></li>';
 		...
 		foreach ($data as $key) {
			...
			if($i <6)
 			{ 	
				...
				echo "<div class='toplvs-listing'><div class='sitethumb-list'><ul>";
 				 //	0 1 2 3 4 5		 
 				echo '<li><img src="http://www.serpavenger.com/serp_avenger/images/screenshots/'.$id.'.jpg" width="120" height="90" ><a href="http://'.$key->domainname.'" target="_blank" ><span>'.$key->domainname.'</span></a></li>';
 				//echo "<br/>"; 	
				if($i==5)
				echo "</div></div></ul>";
 			}
